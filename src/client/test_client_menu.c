...
void register_user(int client_fd)
{
    ...
    scanf("%s", username);
    printf("Enter a new password (min 7 characters): ");
    scanf("%s", password);
...
void start_game(int client_fd)
{
    ...
    write_to_log("Username entered for starting the game:");
    ...
    if (send(client_fd, &msg, sizeof(msg), 0) < 0)
    {
        ...
        write_to_log("Failed to send start game request.");
        return;
    }
    write_to_log("Start game request sent successfully.");

    // Receive the response from the server
    Message response;
    if (recv(client_fd, &response, sizeof(response), 0) < 0)
    {
        perror("Failed to receive start game response");
        write_to_log("Failed to receive start game response.");
        return;
    }
    write_to_log("Start game response received successfully.");
    write_to_log("Response type:");
    write_to_log_int(response.type);

    // Handle the response
    if (response.type == START_GAME_SUCCESS)
    {
        printf("Server: %s\n", response.data);
    ...
    else if (response.type == START_GAME_FAILURE)
    {
        printf("Server: %s\n", response.data);
        printf("Failed to start the game: %s\n", response.data);
        write_to_log("Received START_GAME_FAILURE response from server.");
        write_to_log(response.data);
    }
    else
    {
        printf("Unexpected response type: %d\n", response.type);
        write_to_log("Unexpected response type received:");
        write_to_log_int(response.type);
    }
}
void print_menu()
